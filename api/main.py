...
#     games_list = games_dict["games_list"]
#     print(games_list)
#     recomendations = ["Hello World of Pain"]
#     for game in games_list:
#         recomendations.append(game+"4")
...
def predict(games_list: list[str] = Query(default=None)):
    ...
    for games in gamesList:
        new_row = {'UserID': 128479999, 'Name': games, 'Usage': 'Play', 'HoursPlayed': 999}
        df_steam = df_steam.append(new_row, ignore_index=True)

    print(df_steam.loc[df_steam['UserID'] == 128479999])

    df_games = pd.read_csv('Datasets/games.csv')
    df_games.drop(['main_story', 'main_plus_extras', 'completionist', 'coop', 'versus', 'type', 'developers',
                   'release_na', 'release_eu', 'release_jp', 'platforms', 'publishers'], axis=1, inplace=True)
    df_games['all_styles'].fillna((df_games['all_styles'].mean()), inplace=True)

    df_merge = pd.merge(df_steam, df_games, left_on="Name", right_on="title", indicator=True, how='inner')
    ...
    userId_map, inverse_userId_map = generate_id_mappings(df_merged_games.UserID.unique())
    ...
    R_df = df_exploit_CF.pivot_table(index='mUserId', columns='mGameId', values='rating').fillna(0)
    R = R_df.values
    ...
    already_rated, predictions = recommend_gamesSVD(prediction_df, 128479999, inverse_userId_map, df_exploit_games,
                                                    df_exploit_CF, 10)

    games_list = []
    for title in predictions["Name"]:
        games_list.append(title)
    return {"predictions": games_list}
